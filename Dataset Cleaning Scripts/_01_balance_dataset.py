...
# --- Configuration ---
# 1. SET YOUR SOURCE FOLDER: The folder containing your *disease* folders
SOURCE_DATASET_DIR = Path("Tomato Dataset")

# 2. SET YOUR DESTINATION FOLDER: Where the new, balanced dataset will be created
...
def balance_single_crop_dataset(source_dir, dest_dir, max_files):
    ...
    for disease_folder in source_dir.iterdir():
        ...
        # --- The Core Logic (This is unchanged) ---
        if file_count <= max_files:
            # 4a. Count is at or below the limit. Copy all files.
            print(f"  {disease_folder.name}: {file_count} files. Copying all.")
            for file_path in all_files:
                shutil.copy2(file_path, dest_disease_path / file_path.name)
        
        else:
            # 4b. Count is over the limit. Randomly sample and copy.
...
if __name__ == "__main__":
    # Check if the source path exists before running
