...
# --- Configuration ---
# 1. SET YOUR SOURCE FOLDER: The 256x256 processed dataset
SOURCE_DIR = Path("_03_Normalized Dataset")

# 2. SET YOUR DESTINATION FOLDER: Where the train/val/test splits will go
...
def split_dataset(source_dir, dest_dir, ratios):
    ...
    # --- 2. Pre-scan to find all class folders and count them ---
    ...
    for crop_folder in source_dir.iterdir():
        ...
        for disease_folder in crop_folder.iterdir():
            if disease_folder.is_dir():
                # We store the relative path to maintain structure
    ...
    # --- 3. Process and split each class folder ---
    with tqdm(total=total_classes, unit="class", desc="Splitting Dataset") as pbar:
        for relative_path in class_folders_to_process:
            ...
            # 1. Get all files and shuffle them
            all_files = [f for f in source_class_path.glob('*') if f.is_file()]
            random.shuffle(all_files) # This is critical!
            ...
            # 3. Slice the file list
            train_files = all_files[0:train_split_idx]
            val_files = all_files[train_split_idx:val_split_idx]
            test_files = all_files[val_split_idx:] # The remainder

            # 4. Create the destination sub-folders for this class
